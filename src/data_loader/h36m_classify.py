...
class H36M(data.Dataset):
    def __init__(self, opt, split, train_stats, allowed_subj_list=None):
        ...
        bin_info_file = os.path.join(opt.data_dir, opt.bin_info_pre + (
            '_train' if split == 'train' else '_test') + '.pkl')
        ...
        print('==> loading bin info file {}.'.format(bin_info_file))
        bin_info = {}

        with open(bin_info_file, 'rb') as f:
            bin_info = pickle.load(f)
        f.close()

        for key in bin_info.keys():
            annot[key] = bin_info[key]
        ...
        self.n_bins_x = self.annot['bin_x_oh'].shape[2]
        self.n_bins_y = self.annot['bin_y_oh'].shape[2]
        self.n_bins_z = self.annot['bin_z_oh'].shape[2]
    ...
    def get_inputs(self, index):
        ...
        bin_x = np.copy(self.annot['bin_x'][id])
        bin_y = np.copy(self.annot['bin_y'][id])
        bin_z = np.copy(self.annot['bin_z'][id])

        bin_x_oh = np.copy(self.annot['bin_x_oh'][id])
        bin_y_oh = np.copy(self.annot['bin_y_oh'][id])
        bin_z_oh = np.copy(self.annot['bin_z_oh'][id])

        subj = self.annot['subject'][id]

        return id, img, pose, pose_un, bin_x, bin_y, bin_z, subj

    def __getitem__(self, index):

        shuffled_id = self.shuffle_ids[index]

        id, img, pose, pose_un, bin_x, bin_y, bin_z, subj = self.get_inputs(shuffled_id)
        ...
        return img, bin_x, bin_y, bin_z, meta

    def __len__(self):
        # return self.opt.train_batch
